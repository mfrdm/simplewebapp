...
class VehicleController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','make','initmake','create'),
				'users'=>array('*'),
			),
	...
	public function actionMake()
	{
	    ...
	    $querytxt = Yii::app()->db->createCommand()
    		...
    		->from('tbl_vehiclemake')
    		->where(array('like','name', array('%'.$_GET['q'].'%')))
    		->queryRow();
	    $data=array();
	    ...
	    foreach($querydata as $item){
	    	$data['results'][]=array('id'=>$item['id'],'text'=>$item['name']);
	    }
		echo CJSON::encode($data);
		Yii::app()->end();
	}

	public function actionInitmake()
	{
		if (!YII_DEBUG && !Yii::app()->request->isAjaxRequest) {
        	throw new CHttpException('403', 'Forbidden access.');
    	}
		...
		$querydata = Yii::app()->db->createCommand()
    		->select('id, name')
    		->from('tbl_vehiclemake')
    		->where('id=:id',array(':id'=>(int)$_GET['qid']))
    		->queryRow();
    	$data=array('id'=>$querydata['id'],'text'=>$querydata['name']);
